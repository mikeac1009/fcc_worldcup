...
echo "$($PSQL "SELECT winner.name FROM games JOIN teams AS winner ON games.winner_id = winner.team_id WHERE year = 2014 AND round = 'Eighth-Final' UNION SELECT opponent.name FROM games JOIN teams AS opponent ON games.opponent_id = opponent.team_id WHERE year = 2014 AND round = 'Eighth-Final' ORDER BY name")"
